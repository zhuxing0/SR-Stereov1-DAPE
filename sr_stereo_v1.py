...
from core.uncertaintynet import *
...
class SRStereov2(nn.Module):
    def __init__(self, args):
        ...
        if args.xga_uncertain and args.xga_uncertain_type == 23:
            self.unnet = UnNet1()

        if args.xga_m_map:
            self.m_map_net = MMAPNet()
    ...
    def forward(self, image1, image2, iters=12, disp_gt=None, flow_init=None, test_mode=False):
        ...
        for itr in range(iters): # train: 22, valid: 32
            ...
            if self.args.xga_uncertain and self.args.xga_uncertain_type in [23]:
                uc_map_now, _ = self.unnet(geo_feat)  # torch.Size([4, 1, 80, 184])
                uc_maps.append(uc_map_now)
            ...
            if self.args.xga_uncertain_aft and self.args.xga_uncertain_aft_type in [6]:
                # SR-Stereo_v1
        ...
        return init_disp, disp_preds, uc_maps, delta_disps
